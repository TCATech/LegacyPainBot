...
const { promptMessage } = require("../../functions/rpsfn");
...
module.exports = {
    name: "rps",
    description: "Play Rock Paper Scissors against other people, or even with PainBot!",
    async execute (message, args, client) {
        const member = message.mentions.users.first()
        if(member) return await RockPaperScissors({
            message: message,
            opponent: member,
            ...
            timeEndMessage: "Both of you didn't pick something in time, so I ended the game!",
        ...
        const rpsEmbed = new Discord.MessageEmbed()
            ...
            .setFooter(`${message.member.displayName} vs ${client.user.username}`, message.author.displayAvatarURL({ dynamic: true }))
            .setTitle(`${message.member.displayName} vs. ${client.user.username}`)
            .setDescription("Please react what you would like to choose with the emojis below!")
            .setTimestamp();
