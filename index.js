...
client.error = err => {
  if (typeof(err) === "string")
    return err.replace(/`/g, "`" + String.fromCharCode(8203)).replace(/@/g, "@" + String.fromCharCode(8203));
  else
      return err;
}
...
client.on("message", async message => {
  ...
  if(command.userPerms && !message.member.hasPermission(command.userPerms || [])) return message.channel.send(
    new Discord.MessageEmbed()
    .setTitle('Oopsie Poopsie!')
    .setDescription(`You need the following permissions to use this command: \`${command.userPerms.map((value) => `${value[0].toUpperCase() + value.toLowerCase().slice(1).replace(/_/gi, ' ')}`).join(', ')}\``)
    .setColor(client.color)
    .setFooter(client.user.username, client.user.displayAvatarURL({ dynamic: true }))
    .setTimestamp()
  )
  if(command.botPerms && !message.guild.me.hasPermission(command.botPerms || [])) return message.channel.send(
    new Discord.MessageEmbed()
    .setTitle('Oopsie Poopsie!')
    .setDescription(`Please give me the following permissions: \`${command.botPerms.map((value) => `${value[0].toUpperCase() + value.toLowerCase().slice(1).replace(/_/gi, ' ')}`).join(', ')}\``)
    .setColor(client.color)
    .setFooter(client.user.username, client.user.displayAvatarURL({ dynamic: true }))
    .setTimestamp()
  )
